...
it('should forward select to the underlying findMany when provided', async () => {
	...
	expect(database.tickets.findMany).toHaveBeenCalledWith(expect.objectContaining({ select }));
});
...
it('should not send a select key when none is provided', async () => {
	...
	expect(database.tickets.findMany).toHaveBeenCalledWith(expect.not.objectContaining({ select: expect.anything() }));
});
```
...
describe('AreasRepository', () => {
	...
	beforeEach(async () => {
		...
		const module: TestingModule = await Test.createTestingModule({
			providers: [AreasRepository, { provide: DatabaseService, useValue: database }],
		}).compile();
	...
	describe('findAccountsById', () => {
		...
		it('should call areas.findUnique selecting the linked logins ordered by the relation column', async () => {
			const expected = {
				logins: [{ logins: { id: 'login-a', username: 'admin', email: 'a@b.com' } }],
			};
...
export class AreasRepository {
	...
	async findAccountsById(params: IAreasFindAccountsParams): Promise<IAreasFindAccountsPromise | void> {
...
```ts
import { TPaginationData } from 'prisma-offset-paginator/dist/interfaces';
import { TICKET_PRIORITY, TICKET_STATE } from '../../database/prisma/generated/enums';
...
describe('AreasService', () => {
	...
	describe('findManyWithPagination', () => {
		...
		it('should throw UnprocessableEntityException when the repository returns nothing', async () => {
			...
			await expect(service.findManyWithPagination({ per_page: 10, sort: 'alias' })).rejects.toBeInstanceOf(
...
export class AreasService {
	...
	async findManyWithPagination(params: IAreasFindManyParams): Promise<IAreaListWithPaginationPromise> {
		const { per_page, offset } = params;
		const sort = parseSort(params.sort);

		const repositoryResult = await this.repository.findManyWithPagination<Prisma.areasWhereInput>({
			offset,
			per_page,
			sort,
			where: {},
		...
		if (!repositoryResult) throw new UnprocessableEntityException('repository_error');

		return repositoryResult;
	}

	async findAccountsByAreaId(params: IAreaFindAccountsParams): Promise<IAreaAccountItemListPromise[]> {
	...
	async findTicketsByAreaId(params: IAreaFindTicketsParams): Promise<IAreaTicketListWithPaginationPromise> {
		const { area_id, per_page, offset } = params;
		const sort = parseSort(params.sort);

		const repositoryResult = await this.ticketsRepository.findManyWithPagination<Prisma.ticketsWhereInput>({
			offset,
			per_page,
			sort,
			where: { area_id },
...
): { id: string; role: 'ADMIN' | 'MASTER' | 'USER' }[] => entries.map(([id, role]) => ({ id, role }));
```
...
describe('createOne', () => {
	...
	it('should reject a login whose role is USER', async () => {
		...
		await expect(service.createOne(params)).rejects.toBeInstanceOf(UnprocessableEntityException);
		expect(repository.createOne).not.toHaveBeenCalled();
	});

	it('should reject when one of the logins does not exist', async () => {
		loginsRepository.findManyRolesByIds.mockResolvedValue([{ id: 'login-a', role: 'ADMIN' }]);
	...
	it('should deduplicate repeated login ids before validating and creating', async () => {
		...
		expect(loginsRepository.findManyRolesByIds).toHaveBeenCalledWith(['login-a']);
		expect(repository.createOne).toHaveBeenCalledWith(expect.objectContaining({ login_ids: ['login-a'] }));
	});
...
describe('updateOneById', () => {
	...
	it('should validate the roles before replacing the link set', async () => {
		loginsRepository.findManyRolesByIds.mockResolvedValue([{ id: 'login-a', role: 'MASTER' }]);
		repository.updateOneById.mockResolvedValue({ id: area_id });
	...
	it('should reject a USER role and never reach the repository', async () => {
		loginsRepository.findManyRolesByIds.mockResolvedValue([{ id: 'login-a', role: 'USER' }]);

		await expect(service.updateOneById({ id: area_id, logins: ['login-a'] })).rejects.toBeInstanceOf(
			UnprocessableEntityException,
		);
	...
	it('should throw NotFoundException when the area does not exist', async () => {
		...
		await expect(service.updateOneById({ id: area_id, alias: 'Renamed' })).rejects.toBeInstanceOf(NotFoundException);
		expect(cache.deleteCollection).not.toHaveBeenCalled();
	});
});
```
...
import { IAreaCreateDTO, IAreasListQueryDTO, IAreaTicketsListQueryDTO, IAreaUpdateDTO } from './areas.dto';
...
export class IAreaTicketsListQueryDTO {
	...
	@ApiProperty({
		...
		example: 'created_at',
		type: 'string',
		enum: ['created_at', '-created_at', 'updated_at', '-updated_at', 'priority', '-priority', 'subject', '-subject'],
	})
	@IsString()
	@IsNotEmpty()
	@IsIn(['created_at', '-created_at', 'updated_at', '-updated_at', 'priority', '-priority', 'subject', '-subject'])
	sort: string;
}
...
import { IAreaAccountsListQueryDTO, IAreaCreateDTO, IAreaTicketsListQueryDTO, IAreaUpdateDTO } from './areas.dto';
...
- Produces: `AreasControllerService` with `findAllWithPagination`,
  ...
  `GET areas/:id/tickets`, `POST areas`, `PUT areas/:id`.
...
describe('AreasControllerService', () => {
	...
	describe('createOne', () => {
		...
		it('should propagate errors thrown by the domain service', async () => {
			...
			await expect(controllerService.createOne({ body, ip, account })).rejects.toBe(error);
		});
	});
	...
	describe('updateOneById', () => {
		...
		it('should reject an empty body before reaching the domain service', async () => {
			await expect(controllerService.updateOneById({ id: area_id, body: {}, ip, account })).rejects.toBeInstanceOf(
				BadRequestException,
			);
...
export class AreasControllerService {
	...
	async findAllWithPagination(query: IAreasListQueryDTO): Promise<IAreaListWithPaginationPromise> {
		return await this.areasService.findManyWithPagination({
			...query,
		});
	}

	async findAccounts(params: IAreaAccountsListParams): Promise<IAreaAccountItemListPromise[]> {
	...
	async findTicketsWithPagination(params: IAreaTicketsListParams): Promise<IAreaTicketListWithPaginationPromise> {
		const { area_id, query } = params;
	...
	async createOne(params: IAreaCreateControllerParams): Promise<IAreaCreatePromise> {
		const { body, ip, account } = params;

		const serviceResult = await this.areasService.createOne(body);

		this.logger.log(`[createOne] - LOGINID:${account.id} | AREAID:${serviceResult.id} | IP:${ip} - AREA CREATED`);

		return serviceResult;
	}

	async updateOneById(params: IAreaUpdateControllerParams): Promise<IAreaUpdatePromise> {
		const { id, body, ip, account } = params;

		if (body.alias === undefined && body.description === undefined && body.logins === undefined) {
		...
		this.logger.log(`[updateOneById] - LOGINID:${account.id} | AREAID:${serviceResult.id} | IP:${ip} - AREA UPDATED`);
...
describe('AreasController', () => {
	...
	describe('list', () => {
		it('should delegate the query to the controller service', async () => {
			...
			expect(controllerService.findAllWithPagination).toHaveBeenCalledWith(query);
			expect(result).toBe(expected);
		});
		...
		it('should propagate errors thrown by the controller service', async () => {
			...
			await expect(controller.list({ per_page: 10, sort: 'alias' })).rejects.toBe(error);
		});
	});
	...
	describe('update', () => {
		it('should pass the id param, the body, the ip and the account', async () => {
			...
			const result = await controller.update(account, ip, { id: area_id }, body);
...
export class AreasController {
	...
	@ApiOperation({
		...
		description: 'Return a paginated list of areas with the number of linked logins and tickets.',
	})
	...
	@Get('list')
	async list(@Query() query: IAreasListQueryDTO): Promise<IAreaListWithPaginationPromise> {
		return await this.controllerService.findAllWithPagination(query);
	}

	@ApiOperation({
		summary: 'Get accounts assigned to an area',
		description: 'Return the full list of logins assigned to the given area, without pagination.',
	})
	@ApiBearerAuth('bearer')
	...
	@ApiOperation({
		...
		description: 'Return a paginated list of the tickets assigned to the given area.',
	})
	@ApiBearerAuth('bearer')
	...
	@ApiOperation({
		...
		description: 'Creates an area already linked to one or more ADMIN/MASTER logins.',
	})
	@ApiBearerAuth('bearer')
	...
	@ApiOperation({
		...
		description: 'Updates the alias, the description, or the whole set of logins linked to the area.',
	})
	@ApiBearerAuth('bearer')
